...
from muro.forms import AddNewAnuncio
from muro.models import *
from .forms import *
...
def home(request):
    ...
    paginator = Paginator(anuncio_list, 10)
...
def profileCourses(request):
    if request.user.is_site_admin:
        return redirect(reverse('staffCourses'))
...
def staffCourses(request, course_name=None):
...
def course_homepage(request, course_name):
    ...
    if chapter_list:
        ...
        warning_message = "Actualmente no hay sesiones en este curso."
